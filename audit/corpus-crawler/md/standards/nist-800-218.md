...
EO14028: 4e(ix)
known at all times so that they can be taken into maintain the requirements over time. Example 2: Define policies for securing software development processes
IEC62443: SM-7, SM-9
...
effort can be minimized because the other third-party software components utilized by software being developed.
NISTCSF: ID.GV-3
...
the organization for reuse by the organization’s own with third parties.
EO14028: 4e(vi), 4e(ix)
software. [Formerly PW.3.1] Example 2: Define security-related criteria for selecting software; the criteria can
IDASOAR: 19, 21
...
NIST SP 800-218 SSDF VERSION 1.1
Practices Tasks Notional Implementation Examples References
Example 4: Require third parties to provide provenance5 data and integrity SCAGILE: Tasks Requiring the Help of Security Experts 8
...
responsibilities throughout the SDLC. champions, project managers and leads, senior management, software
...
Example 3: Create a training plan for each role. IEC62443: SM-4
needed.
...
convey that commitment to all with development- releasing software to production and delegating responsibilities as appropriate.
NISTCSF: ID.RM-1, ID.SC-1
related roles and responsibilities. Example 2: Increase authorizing officials’ awareness of the risks of developing
OWASPSAMM: SM1.A
...
formats are to be used. EO14028: 4e(iii), 4e(ix)
...
orchestration.
address a particular part of the SDLC, like a SCSIC: Vendor Software Delivery Integrity Controls
...
SP800181: K0013, K0178
PO.3.3: Configure tools to generate artifacts6 of their Example 1: Use existing tooling (e.g., workflow tracking, issue tracking, value BSAFSS: PD.1-5
...
defined by the organization. actions that are performed for continuous improvement purposes.
...
organization’s expectations by defining and vulnerability severity scores, and other measures for software security.
...
during development. of Done in agile SDLC methodologies).
ISO27034: 7.3.5
MSSDL: 3
...
environments for software development are environments from each other and from production environments, and to
IEC62443: SM-7
strongly protected from internal and external separate components from each other within each non-production environment,
threats to prevent compromises of the in order to reduce attack surfaces and attackers’ lateral movement and
...
the internet to only what is necessary. SP800161: SA-3, SA-8, SA-15
environments.
...
testers, builders, etc.) to perform development-related sensitive data at rest and in transit.
IEC62443: SM-7
tasks using a risk-based approach. Example 2: Configure each development endpoint and the development
...
unauthorized changes to code, both inadvertent based on the principle of least privilege so that only source code intended for public access may need its integrity and availability
CNCFSSCP: Securing the Source Code—Verification, Automation, Controlled
and intentional, which could circumvent or authorized personnel, tools, services, etc. have protected; other code may also need its confidentiality protected.
...
the software. For code that is not intended to be made to the code with accountability to the individual account.
...
time-consuming for attackers to find NISTCSF: PR.AC-4, PR.DS-6, PR.IP-3
code by others.
...
is legitimate and has not been tampered with. consumers’ operating systems or other tools and services can confirm the validity
...
release. provenance data, such as by keeping it in a separate location from the release
IDASOAR: 25
...
release (e.g., in a software bill of materials [SBOM]). formats.
...
requirements for the software; determine what them, including implementing mitigations.
EO14028: 4e(ix)
...
IDASOAR: 1
operation and how the software’s design and protecting sensitive data and safeguarding identification, authentication, and
architecture should mitigate those risks; and access control, including credential management. IEC62443: SM-4, SR-1, SR-2, SD-1
justify any cases where risk-based analysis IR8397: 2.1
...
Example 4: Use data classification methods to identify and characterize each MSSDL: 4
...
IEC62443: SD-1, SD-4
and vulnerability management systems) instead of available (e.g., as configuration-as-code) so developers can readily use them.
...
features and services. [Formerly PW.4.3] OWASPASVS: 1.1.6
...
satisfactorily address the identified risk of the security requirements and satisfactorily
Example 3: Review the software design to confirm that it satisfactorily addresses
information. addresses the identified risk information. ISO27034: 7.3.3
...
(PW.4): Lower the costs of software middleware, frameworks) from commercial, open- in the future, perform the review and evaluation again with that new context in
...
and decrease the likelihood of introducing organization’s software. Example 2: Determine secure configurations for software components, and make
...
analysis, binary software composition analysis) for each software component, and NISTCSF: ID.SC-2
for software that implements security
analyze that information to better assess the risk that the component may OWASPASVS: 1.1.6
...
throughout their life cycles. software components.
...
number of security vulnerabilities in the software, environment to meet the organization’s requirements.
...
Example 3: Regularly validate the authenticity and integrity of compiler, EO14028: 4e(iv), 4e(ix)
reduce costs by eliminating vulnerabilities before
interpreter, and build tools. See PO.3.
testing occurs. IEC62443: SI-2
MSSDL: 8
...
approved configurations. treated as errors and eliminated except those determined to be false positives or
EO14028: 4e(iv), 4e(ix)
irrelevant.
IEC62443: SI-2
...
Compliance with Security Requirements analysis (tools are used to find issues in code, either in party code and reusable code modules written in-house.
...
NISTLABEL: 2.2.2.2
can be corrected before the software is released should be used, as defined by the organization. analysis should be performed and how it should be conducted.
...
methods lowers the effort and resources needed SP80053: SA-11
the software.
...
Security Requirements (PW.8): Help identify identified by previous reviews, analysis, or testing and, environment). This may include third-party executable code and reusable
EO14028: 4e(ix)
...
before the software is released in order to Example 2: Choose testing methods based on the stage of the software.
...
Example 3: Incorporate tests for previously reported vulnerabilities into the EO14028: 4e(iv), 4e(v), 4e(ix)
team’s workflow or issue tracking system.
project’s test suite to ensure that errors are not reintroduced. IDASOAR: 7, 8, 10, 11, 38, 39, 43, 44, 48, 55, 56, 57
...
the software at the time of installation to reduce or a security-related setting so that the default settings weaknesses, operational issues, or other problems.
EO14028: 4e(iv), 4e(ix)
the likelihood of the software being deployed are secure and do not weaken the security functions
IDASOAR: 23
with weak security settings, putting it at greater provided by the platform, network infrastructure, or
...
Identify and Confirm Vulnerabilities on an RV.1.1: Gather information from software acquirers, Example 1: Monitor vulnerability databases9, security mailing lists, and other BSAFSS: VM.1-3, VM.3
...
opportunity for attackers. IEC62443: DM-1, DM-2, DM-3
...
responsibilities, and processes needed to support that vulnerabilities.
EO14028: 4e(viii), 4e(ix)
policy. Example 2: Have a Product Security Incident Response Team (PSIRT) and
...
o
• References
Added CNCFSSCP, IEC62443, ISO29147, ISO30111, NISTDVS,
o
...
Retired identifiers for deleted/moved practices and tasks (PW.3, PW.3.1, PW.3.2,
o
PW.4.3, and PW.5.2)
Added colored borders and shaded rows for each group of practices; indicated
o
